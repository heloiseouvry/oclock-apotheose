...
const SalaryReport = () => {
  ...
  const [startDate, setStartDate] = useState(
    `${new Date().getFullYear()}-${("0" + currentMonth).slice(-2)}-01`
  );
  const [endDate, setEndDate] = useState(
    `${new Date().getFullYear()}-${("0" + nextMonth).slice(-2)}-01`
  );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.get(
        `${admin_url}/userssalary/?start_date=${startDate}&end_date=${endDate}`,
        {
        ...
        <Grid textAlign="center" columns="equal" celled="internally">
          ...
          {salaryData.map((tech) => (
            <Grid.Row key={tech.id}>
              ...
              {selectedDates.map((date) => (
                <Grid.Column key={date}>
                  {tech.phases.find((elem) => {
                    if (elem.start_date === date) {
                      phase = elem;
                      return elem;
                    }
                  }) ? (
